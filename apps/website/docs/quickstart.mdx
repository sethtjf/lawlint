...
from a terminal, lawlint opens the local browser workspace instead. Use
`lawlint app contract.docx` to open the workspace with a document already
loaded.
...
Pipes remain a normal CLI path; only an interactive terminal with no arguments
