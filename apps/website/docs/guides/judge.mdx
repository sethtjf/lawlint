...
| | Hosted (`anthropic:`, `openai:`, `foundry:`) | `local:` |
| --- | --- | --- |
| Text per request | 24,000 chars — most documents in one | 1,200 chars |
| Requests | One per rule | One per section, all rubrics together |
| In flight at once | 4 | 1 |

Hosted models get the whole document per rule, so a rule can see across
sections. Local models keep small sections with the rubrics bundled, because a
